const { aggregatorwiseReportService, datewiseReportService, answiseReportService, cliwiseReportService, messagetypewiseReportService } = require("../services/dippingReport.service");
...
exports.messagetypewiseReport = async (req, res) => {
    try {
        const filter = JSON.parse(req.query?.filter) || {};
        ...
        const messagetypewiseReport = await messagetypewiseReportService(filter);

        if (messagetypewiseReport.length > 0) {
            res.status(200).json({
                status: "Success",
                data: messagetypewiseReport
            })
        }
        else {
            res.status(400).json({
